...
def train_net_fedrom(net, train_dataloader, epochs, args, device):
    """Local training with dual-GMM reliability: loss-GMM ∩ center-distance-GMM."""
    net.cuda()
    net.train()
...
def fedrom_alg(args, n_comm_rounds, nets, global_model, party_list_rounds, net_dataidx_map, train_local_dls, test_dl, traindata_cls_counts, moment_v, device, global_dist, logger):
    best_test_acc = 0
    best_f1 = 0
    best_precision = 0
    best_recall = 0
    ...
    for round in range(warmup_rounds, n_comm_rounds):
        ...
        for net_id, net in nets_this_round.items():
            train_dl_local = train_local_dls[net_id]
            train_net_fedrom(net, train_dl_local, args.epochs, args, device)
        total_data_points = sum([len(net_dataidx_map[r]) for r in party_list_this_round])
        fed_avg_freqs = [len(net_dataidx_map[r]) / total_data_points for r in party_list_this_round]
        global_w = None
        for idx, net in enumerate(nets_this_round.values()):
